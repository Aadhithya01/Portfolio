        ...
        <RevealGroup className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {projects.map((project) => (
            <motion.a
              ...
              whileHover={{ y: -8 }}
              className="gradient-border group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-card/60 p-6 backdrop-blur transition-shadow hover:shadow-2xl hover:shadow-accent/10"
            >
              {/* Hover spotlight */}
              ...
              <div className="mb-4 flex items-center justify-between">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-accent/10 text-accent transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6">
              ...
              <h3 className="mb-2 font-headline text-xl font-semibold text-foreground transition-colors group-hover:text-accent">
                {project.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {project.tech.map((tech) => (
                  <Badge
                    key={tech}
                    variant="outline"
                    className="border-accent/30 bg-accent/5 text-xs text-foreground/80"
                  >
                    {tech}
                  </Badge>
                ))}
